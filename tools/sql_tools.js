...
postTime = postTime * 0.001;
postTime = Math.trunc(postTime);
    db.connect().execute(`insert into Content (user_id, content, post_time, time_of_del) values (?, ?, ${postTime}, ${postTime});`, data, (err, results, fields) => {
        console.log(results);
        console.log(fields);
        if (err === null) {
            res.json(results);
